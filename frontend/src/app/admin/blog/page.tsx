  ...
  return (
    ...
    >
      {loading ? (
        ...
        <div className="rounded-[20px] bg-surface/50 backdrop-blur-md shadow-soft border border-white/10 divide-y divide-white/5">
          {posts.map((p) => (
            <div key={p.id} className="flex items-center gap-4 p-4 hover:bg-white/5 transition-colors">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button type="button" onClick={() => setDeleteId(p.id)} className="p-2 rounded-lg hover:bg-red-500/10 text-red-500"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed inset-4 lg:inset-10 bg-surface border border-white/10 rounded-[20px] shadow-2xl z-50 overflow-y-auto">
              <div className="p-6 lg:p-8 max-w-3xl mx-auto space-y-5">
                <div className="flex justify-between">
                  <h2 className="font-display text-lg font-semibold text-text-primary">{editing ? 'Edit Post' : 'New Post'}</h2>
                  <button type="button" onClick={() => setOpen(false)}><X className="w-5 h-5" /></button>
                </div>
                <input className="input-field bg-white/5 border-white/10 text-text-primary" placeholder="Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value, slug: editing ? form.slug : slugify(e.target.value) })} />
                <input className="input-field bg-white/5 border-white/10 text-text-primary" placeholder="Slug" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} />
                ...
                <input className="input-field bg-white/5 border-white/10 text-text-primary" placeholder="Tags (comma separated)" value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} />
